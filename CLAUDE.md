...
### Frontend Stack
- **Tailwind CSS 4** with Vite integration via `@tailwindcss/vite`
- **Blade templates** using layouts/partials pattern
- **Lucide icons** loaded from CDN
- Custom color theme defined in `resources/css/app.css` using CSS custom properties (`--color-villa-*`)

### Template Structure
- `resources/views/layouts/app.blade.php` - Main layout with navigation, footer, and JS initialization
- `resources/views/home.blade.php` - Homepage assembling all partials
- `resources/views/partials/` - Reusable sections (menu, gallery, reservations, contact, etc.)

### Custom Theme Colors
The brand palette is defined in `resources/css/app.css`:
- `villa-charcoal`, `villa-espresso`, `villa-coffee` - Dark tones
- `villa-ember`, `villa-flame` - Orange accent colors
- `villa-gold` - Highlight color
- `villa-cream` - Light text/backgrounds

### Key Configuration
- Uses SQLite in-memory database for testing (`phpunit.xml`)
- Tests configured in `tests/Unit/` and `tests/Feature/` directories
- Vite entry points: `resources/css/app.css` and `resources/js/app.js`

## Testing

Tests use PHPUnit with Laravel's testing helpers. Run a single test file:
